  ...
  m_outputFilename (filename)
{
  ...
  // Empty file
  //std::ofstream myfile;
  //myfile.open (m_outputFilename.c_str(), std::ofstream::app);
  //myfile.close ();
}
                                         ...
                                         PhyPacketData packetTracker)
{
  ...
  */
  std::ofstream myfile;
  myfile.open (m_outputFilename.c_str(), std::ofstream::app);
  //"PHY Total, PHY Successful, PHY Interfered, PHY No More Receivers, PHY Under Sensitivity, PHY Lost Because TX
  myfile <<
		  performancesAmounts.at(0) << "," <<
		  performancesAmounts.at(1) << "," <<
		  performancesAmounts.at(2) << "," <<
